...
"""
This function parses infomration that is on the OSURC website that was generated by 
a website for the purposes of being read by this script. 

This function returns a dictionary that contains the information outputted by the
web script
"""
def getData():
...
if __name__ == "__main__":
	...
	if(len(sys.argv) > 1): #Command line arg recieved
		if(sys.argv[1] == "vars"):  #Output the variables that can be used
			...
			print data
			sys.exit() #Exits so that the variables can be referenced
		else:
			print "That is an invalid command line argument. Please try again."
	
	#Basic test to output #'s for every status
	print "Begin Test:"	
	while 1:
		data = getData() 
